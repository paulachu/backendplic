...
public class SkyboxResource {
    ...
    @Consumes(MediaType.MULTIPART_FORM_DATA)
    @Transactional
    public Response putSkybox(@PathParam("id") Long id, @Valid @Parameter(hidden = true) @MultipartForm AddSkyboxRequest addSkyboxRequest){
        try {
            SkyboxEntity skyboxEntityToAdd = new SkyboxEntity().withFilename(addSkyboxRequest.getFilename()).withFile(addSkyboxRequest.getFile());
            SkyboxEntity skyboxEntityAdded = skyboxService.putSkybox(skyboxEntityToAdd, id);
            return Response.status(Response.Status.CREATED).entity(entityToAddResponse.convert(skyboxEntityAdded))
                    .build();
        } catch (Exception e) {
            e.printStackTrace();
